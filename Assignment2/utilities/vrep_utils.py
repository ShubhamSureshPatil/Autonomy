...
ARM_CUBOID_NAMES = ['shoulder_link_collision_cuboid',
...
def get_arm_cuboid_handles(clientID):
    global ARM_CUBOID_HANDLES
    if not ARM_CUBOID_HANDLES:
        # Cache arm joint handles to avoid repeated handle requests
        ARM_CUBOID_HANDLES = [get_handle_by_name(clientID, j) for j in ARM_CUBOID_NAMES]
    return ARM_CUBOID_HANDLES

def get_obstacle_cuboid_handles(clientID):
    global OBSTACLE_CUBOID_HANDLES
    if not OBSTACLE_CUBOID_HANDLES:
        # Cache arm joint handles to avoid repeated handle requests
        OBSTACLE_CUBOID_HANDLES = [get_handle_by_name(clientID, j) for j in OBSTACLE_CUBOID_NAMES]
